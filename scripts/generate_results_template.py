...
def build_results_sheet(wb):
    ...
    # 컬럼 너비
    col_widths = [8, 10, 16, 12, 12, 12, 14, 8, 14, 12, 12, 8, 12]
    for i, w in enumerate(col_widths, 1):
        ws.column_dimensions[get_column_letter(i)].width = w

    # 헤더 행 (1행)
    headers = ['순위', '차량번호', '팀명', '드라이버1', '드라이버2', '드라이버3',
    ...
    # 예시 행 데이터 (2~4행)
    sample_rows = [
        [1, '7',  '팀스피드웨이', '김태환', '이인용', '',       '아반떼 N',   25, '0:45:12.345', '',        '1:45.678', 25, 'classified'],
        [2, '88', 'AXIS Racing',  '이운영', '홍준기', '조예찬', '벨로스터 N', 25, '0:45:18.901', '+6.556',  '1:46.123', 18, 'classified'],
...
def build_guide_sheet(wb):
    ...
    guide_rows = [
        ...
        ('드라이버3',  '선택', '텍스트',                  '공동 드라이버 3번째. 3인 참가 시 입력',                                                          '박재현'),
