...
export default function GalleryImportR4Page() {
  ...
  async function load() {
    setError('')
    const res = await fetch('/api/admin/gallery-import-r4')
  ...
  if (!authed) {
    return (
      <main style={{ maxWidth: 400, margin: '80px auto', padding: 24, fontFamily: 'sans-serif' }}>
        <h1 style={{ fontSize: 20, marginBottom: 16 }}>R4 갤러리 업로드</h1>
  ...
  return (
    <main style={{ maxWidth: 560, margin: '40px auto', padding: 24, fontFamily: 'sans-serif' }}>
      <h1 style={{ fontSize: 22, marginBottom: 6 }}>2026 R4 갤러리 업로드</h1>
      <p style={{ color: '#666', fontSize: 14, marginBottom: 16 }}>
        R4 오피셜포토 <b>{total}장</b>을 Sanity 갤러리로 업로드합니다. 10장씩 배치로 진행되며, 완료까지 창을 닫지 마세요. 다시 실행하면 처음부터 새로 덮어씁니다.
      </p>
