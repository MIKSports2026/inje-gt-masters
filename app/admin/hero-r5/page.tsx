...
export default function HeroR5Page() {
  ...
  async function run() {
    ...
    try {
      const res = await fetch('/api/admin/hero-r5', { method: 'POST' })
  ...
  if (!authed) {
    return (
      <main style={{ maxWidth: 400, margin: '80px auto', padding: 24, fontFamily: 'sans-serif' }}>
        <h1 style={{ fontSize: 20, marginBottom: 16 }}>R5 키비쥬얼 → 히어로 1번</h1>
  ...
  return (
    <main style={{ maxWidth: 560, margin: '40px auto', padding: 24, fontFamily: 'sans-serif' }}>
      <h1 style={{ fontSize: 22, marginBottom: 6 }}>R5 키비쥬얼을 홈 히어로 1번 슬라이드로</h1>
      <p style={{ color: '#666', fontSize: 14, marginBottom: 16 }}>
        R5 키비쥬얼(THE FINAL STINT : Endure to glory)을 홈 히어로 <b>맨 앞 슬라이드</b>로 추가합니다. 데스크톱(16:9)·모바일(세로) 이미지가 함께 등록됩니다. 여러 번 눌러도 중복되지 않습니다.
      </p>
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/r5-hero/desktop.jpg" alt="R5 데스크톱" style={{ width: 220, borderRadius: 6, border: '1px solid #eee' }} />
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/r5-hero/mobile.jpg" alt="R5 모바일" style={{ width: 90, borderRadius: 6, border: '1px solid #eee' }} />
      </div>
      ...
      {done && <p style={{ background: '#e8f5e9', border: '1px solid #a5d6a7', padding: 12, fontSize: 14, marginTop: 12 }}>✅ 완료 — 홈 히어로 첫 슬라이드로 R5 키비쥬얼이 등록됐습니다. (반영 최대 1분)</p>}
    </main>
  )
}
