// app/api/admin/gallery-import-r4/route.ts — R4 갤러리 사진 배치 업로드
import { NextResponse } from 'next/server'
import { cookies } from 'next/headers'
import crypto from 'crypto'
import manifest from '@/data/r4-gallery.json'

const PID = process.env.NEXT_PUBLIC_SANITY_PROJECT_ID ?? 'cq465tvw'
const DS = 'production'
const V = '2024-01-01'
const DOC_ID = 'media-r4-gallery'
const ROUND = 'round-2026-r4'
...
export async function POST(req: Request) {
  ...
  for (let i = 0; i < batch.length; i++) {
    const idx = offset + i
    const imgRes = await fetch(`${origin}/${FOLDER}/${batch[i].file}`, { cache: 'no-store' })
    ...
    photoObjs.push({
      ...
      alt: `2026 인제 GT 마스터즈 4라운드 오피셜 포토 #${idx + 1}`,
      caption: '', credit: '',
    })
  }
