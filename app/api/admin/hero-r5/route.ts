// app/api/admin/hero-r5/route.ts — R5 키비쥬얼을 홈 히어로 1번 슬라이드로 등록
...
async function uploadAsset(origin: string, file: string, token: string): Promise<string> {
  const img = await fetch(`${origin}/r5-hero/${file}`, { cache: 'no-store' })
  if (!img.ok) throw new Error(`이미지 로드 실패 ${file} (${img.status})`)
  const buf = Buffer.from(await img.arrayBuffer())
  const up = await fetch(`https://${PID}.api.sanity.io/v${V}/assets/images/${DS}?filename=r5-${file}`,
...
export async function POST(req: Request) {
  ...
  const slide = {
    ...
    alt: '5라운드 키비쥬얼', isActive: true,
  }
  // 기존 R5 슬라이드 제거 후 맨 앞에 삽입 (재실행 안전)
  const mutations = [
