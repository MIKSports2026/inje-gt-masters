...
export default function EntryForm({ isOpen, rounds, initialRoundNumber }: Props) {
  ...
  return (
    <div className="ef-card">
      ...
      {/* 참가서약서 모달 (인라인) */}
      {showPledge && (
        <div onClick={() => setShowPledge(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.88)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ width: '90%', maxWidth: '580px', maxHeight: '80vh', background: '#1A1A1A', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--font-heading)', fontSize: '1rem', letterSpacing: '2px', color: '#fff', textTransform: 'uppercase' }}>참가 서약서</span>
              <button onClick={() => setShowPledge(false)} style={{ background: 'none', border: 'none', color: '#aaa', fontSize: '1.2rem', cursor: 'pointer' }}>✕</button>
            </div>
            <div style={{ padding: '24px 32px', overflowY: 'auto', flex: 1 }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
                  <span style={{ fontFamily: 'var(--font-heading)', color: '#E60023', fontSize: '0.85rem', flexShrink: 0, marginTop: '2px' }}>{i + 1}.</span>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', color: '#AAA', lineHeight: 1.8, margin: 0, wordBreak: 'keep-all' }}>{item}</p>
                </div>
              ))}
            </div>
            <div style={{ padding: '20px 32px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
              <button onClick={() => { set('agreedRules', true); setShowPledge(false) }} style={{ width: '100%', background: '#E60023', color: '#fff', fontFamily: 'var(--font-heading)', fontSize: '1rem', fontWeight: 700, letterSpacing: '3px', padding: '16px', border: 'none', cursor: 'pointer', textTransform: 'uppercase' }}>동의합니다</button>
            </div>
          </div>
        </div>
      )}
